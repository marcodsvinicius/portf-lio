...
* **Hero:** `#hero-canvas` draws a lime node mesh — a jittered lattice of nodes joined by faint edges — with signal pulses travelling from node to node, lighting each edge behind them and firing a ring when they land. The pointer brightens and pushes nearby nodes, `.hero-spot` is a radial light following the cursor, and a cached radial vignette darkens the centre so the headline stays readable. Base edges are batched into a single stroke and base nodes into a single fill, so a full-screen mesh still runs at 60 fps. Under reduced motion the mesh renders once and never animates. `.hero-highlight` wraps the key phrase of the `h1` (lime, glow, underline that draws itself). `.scroll-cue` is the animated scroll hint (hidden after 80 px, not shown on phones).
...
* **AI First:** one `.ai-panel` holds everything — a bar with the `.ai-step` tracks and the `01/04` counter, the `.ai-console` that types the active card's `data-prompt` and reveals its `data-out`, and the four `.ai-card` capabilities flush inside the same frame. The active step's fill is a Web Animations timeline whose duration is `text.length * 24ms + 380 + 3400`; the typing, the output and the auto-advance are all read from its `currentTime`, so pausing that one animation pauses the whole sequence. Clicking a track jumps to that capability, and hovering a card (desktop) shows it at once and holds until the pointer leaves. Articles render from the `#ai-articles` JSON block into `#ai-articles-list`; an empty list falls back to a single card linking to the Medium profile.
